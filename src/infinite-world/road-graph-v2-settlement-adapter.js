...
} from './road-graph-v2.js';

export const ROAD_GRAPH_V2_SETTLEMENT_TEMPLATE_SCHEMA = 'w5-road-graph-v2-settlement-template-1';
...
export async function createRoadGraphV2SettlementTemplate({
  worldSeedHash,
  candidate,
  connectivityGraph,
  roadTimingRun = null,
  ...
  const graph = await ROAD_GRAPH_V2_EXPERIMENTAL_STAGE_GENERATOR_REGISTRY.get(
    'roadGraph',
    ROAD_GRAPH_V2_GENERATOR_ID,
  ).generate({
    worldSeedHash,
  ...
  const blocks = await ROAD_GRAPH_V2_EXPERIMENTAL_STAGE_GENERATOR_REGISTRY.get(
    'block',
    'block-v1',
  ).generate({ worldSeedHash, roadGraph: graph });
  ...
  return Object.freeze({
    schemaVersion: ROAD_GRAPH_V2_SETTLEMENT_TEMPLATE_SCHEMA,
    settlementId: candidate.settlementId,
    settlementType: candidate.settlementType,

...
| Human population and AI | `src/game.js` Human spawn/update paths | matched by P3/P6, PO review pending | High-quality population remains independent of W8 Quality. Stable-ID decisions reproduce finite idle, flee variation, trip/fallen/recover, building avoidance, and tangent water avoidance with frame-rate-independent probabilities; residents now spawn outside their owner Building collision. v5 persists the complete behavior state and older Saves initialize it neutrally | Gameplay and Save round-trip/migration tests; PO settlement review |
...
| Boss | `src/game.js`, `src/constants.js` Boss paths | matched by P5/P6, PO review pending | Natural and Developer Tools spawn share the protected slither/sweep/charge/dig/breach/recover contract and 14 segments. Tail contact/cooldown, predicted Breach, one-shot landing AoE/push/Scar, 15-shot hyper-rage spray, slither/recover Acid cadence, finite Acid flight/collision, 6 Damage, 1.1s Debuff with 0.75 movement, segment breaks, death cloud, and next threshold are connected to bounded presentation, Audio, and v5 continuity | Nuclear/Boss, Combat, Experience Shell, Production Visuals and Save tests; PO Boss run |
| Buildings and World Objects | `src/game.js`, settlement visual/life modules | matched by P3/P6, PO review pending | Existing Building values including Barn/Factory/Haystack/Cow remain unchanged. Generated grass, flower, shrub, street lamp, and road sign use formal World Detail descriptors instead of `tree`; the finite-destructible road sign uses its Scale gate, radius, one-hit destruction, colored debris, and Stable-ID destruction state. Destroyed Stable IDs reconstruct their Ruin/Scorch without replaying transient debris | Gameplay, parity Chunk, Production Visuals, Save and revisit tests; PO review |
| Combat and destruction | `src/game.js`, `src/constants.js`, `src/scale-sandbox.js` | matched by P4/P6, PO review pending | Damage, Heal, Score, Hit Stop, Shake, finite High-quality impact/death Particle counts, Rock/Building/Tank Debris, Human blood/Shockwave, Scorch/Blood Scar, bounded Ruins, and Atomic output/lifetimes use the protected finite values in fixed resource pools. Continue/revisit rebuilds only durable Ruin/Scorch | Combat, destruction, resource lifecycle and revisit tests; PO 5-minute review |
| HUD and settings | `index.html`, `src/game.js` | matched by P6 with intentional W8 menu differences, PO review pending | New Game/Continue and functional W8 controls are retained. Finite functional HP, Score, Scale, Atomic, Boss, News, Game Over, Mouse, Volume, Shake, FPS, Quality, and persisted reload-applied antialias behavior are present; inactive placeholders remain omitted | Experience Shell and Save tests; PO review |
...
| Save and revisit | finite has no equivalent Infinite persistence; W8 v5 contract | matched by P6 as an intentional W8 extension | v5 persists Player Acid debuff, Human trip/water behavior, Boss Tail/recover/landing sequence, and antialias preference. v4, v3, W7, and W6 migrate through a validated temporary candidate with neutral new state; incomplete/corrupt v5 is rejected before live mutation. Durable Ruin/Scorch rebuilds from Stable-ID destruction while transient Particle/cache state is not saved | v5 round-trip, v4/v3/W7/W6 migration, corruption, and revisit tests |
